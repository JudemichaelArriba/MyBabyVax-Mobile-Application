            ...
            if (result.resultCode == RESULT_OK) {
                ...
                authServices.handleSignInResult(data) { credential, error ->
                    if (credential != null) {
                                ...
                                override fun onDataChange(snapshot: DataSnapshot) {
                                    if (snapshot.exists()) {
                                        ...
                                        userRef.setValue(newUser).addOnSuccessListener {
                                            ...
                                            binding.loadingOverlay.visibility = View.GONE
                                            Toast.makeText(this@MainActivity, "Failed to create user: ${e.message}", Toast.LENGTH_SHORT).show()
                                        }
                                    }
                                }
                        ...
                        Toast.makeText(this, "Failed: ${error ?: "Unknown error"}", Toast.LENGTH_SHORT).show()
                        binding.loadingOverlay.visibility = View.GONE
                    }
                }
            } else {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.loginBtn.setOnClickListener {
            ...
            if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
                ...
                binding.usernameInputLayout.boxStrokeColor = getColor(R.color.error)
                binding.loadingOverlay.visibility = View.GONE
                return@setOnClickListener
            } else {
                binding.usernameInputLayout.error = null
                binding.usernameInputLayout.boxStrokeColor = getColor(R.color.mainColor)
            }
                    ...
                    databaseService.fetchUserById(user.uid, object : InterfaceClass.UserCallback {
                        override fun onUserLoaded(userData: Users) {
                            if (userData.role == "User") {
                                ...
                                DatabaseService.saveFcmToken()
                                binding.loadingOverlay.visibility = View.GONE
                                DialogHelper.showSuccess(this@MainActivity, "Login Successful", "Welcome back!") {
                                    val intent = Intent(this@MainActivity, bottomNav::class.java)
                                    startActivity(intent)
                                    finish()
                                }
                                ...
                                binding.loadingOverlay.visibility = View.GONE
                                DialogHelper.showError(this@MainActivity, "Access Denied", "Only user accounts can log in through this app.")
                            }
                        }
